...
const UserProfile = () => {
    ...
    const handleImageClick = (index) => {
        axios.post("http://192.168.0.202:1111/viewpost", {user: username, file: userposts[index]})
        .then((response) => {
                ...
                {userposts.map((post, index) => {
                    return (
                        <div key={index} className="post-image">
                            <img src={`http://192.168.0.202:1111/${post.split('/').pop()}`} alt={`Post ${index + 1}`} onClick={() => handleImageClick(index)} />
                        </div>
                    );
                })}
            </div>
